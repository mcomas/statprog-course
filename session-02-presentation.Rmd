...
## Selecting observations {.smaller}

* `slice()` (helpers `slice_head()`, `slice_tail()`, `slice_min()`, `slice_max()`, `slice_sample()`)
...
## Transform variable (`mutate()`) {.smaller}
...
* `sample_n()`, `sample_frac()`. Sample from from a table.
...
* To understand the distribution of a categorical variable, we need the frequencies: either absolute or relative.
...
* To understand the distribution of a categorical variable, we need the frequencies of its categories: either absolute or relative.
* How is the distribution of `origin`?
...
* To understand the distribution of a numerical variable, the most common aspect are: the location and the variability of its values. The skewness can be of interest too.
* Location. 
    * Mean (`mean()`)
...
* Correlation can be easily misinterpreted.
  * To infer causality, we need to be free of confounding. [Causal inference](https://www.bradyneal.com/causal-inference-course)
